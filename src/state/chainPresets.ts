...
export interface ChainPresets {
  [chainId: string]: Readonly<ChainPreset>
}

export const chainPresets: ChainPresets = [
  eosMainnet,
  telosMainnet,
  worbliMainnet,
  waxMainnet,
  ...
  bosTestnet,
  meetoneTestnet,
].reduce((acc: ChainPresets, chain: Readonly<ChainPreset>) => {
  acc[chain.id] = chain
  return acc
}, {})
